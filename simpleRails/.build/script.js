...
})(window, window.angular);;angular.module('phonecatApp', ['ui.router'])
...
.config(function($routeProvider) {
	...
	$routeProvider
		// phones routes
		.when('/phones', {
			templateUrl: 'phones/index.html', controller: 'PhoneIndexCtrl'})
		.when('/phones/new', {
			templateUrl: 'phones/new.html', controller: 'PhoneNewCtrl'})
...
.controller('PhoneIndexCtrl', function ($scope, phonesData, indexOfByID) {
  $scope.phones =  phonesData;

  $scope.removePhone = function(phone){
    phonesData.splice(indexOfByID(phone, phonesData)[0], 1);
  };

})

.controller('newPhoneCtrl', function ($scope, phonesData) {

	$scope.submitNewPhone = function(){
		var data = {name: $scope.name, snippet: $scope.snippet, id: phonesData.reverse()[0].id+1};
		phonesData.push(data);
	};
  ...
  return {
      ...
      link: function(scope, element, attrs){
        var myElement = angular.element(element)[0],
            toggleBtn = angular.element(myElement.querySelectorAll('.toggleBtn')),
            toggleArea = angular.element(myElement.querySelectorAll('.toggleArea'));
...
.filter('search', function(){
	return function(data, text){
        return data.filter(function(data){
            if (!text) { return true; }
            if (data.name.toLowerCase().indexOf(text) > -1 || data.snippet.toLowerCase().indexOf(text) > -1) {
                return true;
            }
        });
    };
...
.filter('toArray', function(){
  return function(obj){
    if (obj.constructor.name === "Array") { return obj; }
    // Object.keys(obj)
  };
  ...
  $templateCache.put("phones/index.html",
    ...
    "\n" +
    "  <ul>\n" +
    "    <li ng-repeat=\"phone in phones  | search:query | orderBy:orderProp\" >\n" +
    "      <div toggle-phone>\n" +
    "        <input type=\"button\" value=\"Hide\" class=\"toggleBtn\"/>\n" +
    "        <div class=\"toggleArea\">\n" +
    "          <p>{{phone.name}}</p>\n" +
    "          <p>{{phone.snippet}}</p>\n" +
    "          <input type=\"button\" value=\"delete\" class=\"delete\" ng-click=\"removePhone(phone)\"/>\n" +
    "        </div>\n" +
    "      </div>\n" +
    "    </li>\n" +
    "  </ul>\n" +
  ...
  $templateCache.put("phones/new.html",
    "<div ng-controller=\"newPhoneCtrl\" newPhone class=\"rounded-frame\">\n" +
    "  Name <input ng-model=\"name\" value=\"name\" class=\"user-input\"/>\n" +
    "  Snippet <input ng-model=\"snippet\" value=\"snippet\" class=\"user-input\"/>\n" +
    "  <span ng-click=\"submitNewPhone(name, snippet)\">click me</span>\n" +
    "</div>"
  );
